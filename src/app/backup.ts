...
// with any text editor and put back with the same code path a folder or
// Dropbox pull uses.
//
...
export function backupFileName(today = dayKeyOf(new Date())): string {
  return `invoice-backup-${today}.json`;
}
 ...
 * and keeps every record it can, which is the right outcome for a restore.
 */
