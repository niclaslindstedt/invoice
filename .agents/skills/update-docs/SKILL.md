...
Keeps `docs/` honest (OSS_SPEC §11.1). The docs in this repo describe _behaviour that is derived_, not stored — so a one-line change to a rule in `invoice.ts` or in a region's `check` can silently falsify a paragraph in `docs/invoice-model.md` without breaking a single test. That is the specific drift this skill exists to catch.
...
4. Pay special attention to numbers and rules quoted in prose: the VAT rates, the default due days, how the total is rounded, which sections a region requires, which identifiers are required, the debounce interval, what a region's `check` refuses. These are the values most likely to be edited in code and forgotten in prose.

## Mapping

| Changed source                                              | Doc to update                                                                                                 |
| ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| `src/app/invoice.ts`                                        | `docs/invoice-model.md`; `docs/features/invoices.md`; the README's Examples block if the returned shape moved |
| `src/app/regions/**`                                        | `docs/regions.md`; `docs/features/company.md`                                                                 |
| `src/app/layout.ts`, `InvoiceEditor.tsx`, `InvoicePage.tsx` | `docs/features/editor.md`; `docs/features/templates.md`                                                       |
| `src/app/revisions.ts`, `HistoryModal.tsx`                  | `docs/features/customers.md`; `docs/invoice-model.md` (the history section)                                   |
| `src/app/interchange.ts`                                    | `docs/interchange.md`                                                                                         |
| `src/app/merge.ts`, `src/app/useSyncEngine.ts`              | `docs/sync.md`; `docs/features/storage.md`                                                                    |
...
| A new localStorage or IndexedDB key anywhere                | `docs/configuration.md` (storage keys table)                                                                  |
