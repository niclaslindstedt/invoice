...
4. Pick the `type` from what the change did to the _user's_ experience, not from the commit type:
   ...
   `breaking: true` forces a major bump — for this app that means a document-format change an older build cannot read, a change to the interchange format the Time app writes, or a removed setting.

## Mapping

| Changed source                                                | Fragment needed                                 |
| ------------------------------------------------------------- | ----------------------------------------------- |
| `src/app/*Screen.tsx`, `InvoiceEditor.tsx`, `InvoicePage.tsx` | Yes — the UI moved                              |
| `src/app/invoice.ts`, `layout.ts`, `regions/**`               | Yes — the figures or rules a user reads changed |
| `src/app/useSyncEngine.ts`, `merge.ts`                        | Yes — storage behaviour is user-visible         |
| `src/app/types.ts`, `migrations.ts`, `interchange.ts`         | Yes, and consider `breaking: true`              |
| `src/app/i18n/*.ts` alone                                     | Only if the wording change alters meaning       |
