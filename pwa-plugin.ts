...
// visibly separate home-screen tiles instead of three identical "Invoice"
...
function channelName(base: string): { name: string; short_name: string } {
  if (base === "/preview/")
    return { name: "Invoice (preview)", short_name: "Invoice pre" };
  if (base === "/branch/")
    return { name: "Invoice (branch)", short_name: "Invoice br" };
  return {
    name: "Invoice",
    short_name: "Invoice",
  };
...
export function buildManifest(base: string): string {
  ...
  const manifest = {
    ...
    short_name,
    description:
      "A privacy-first invoicing PWA: your customers, your company details and your invoices, laid out on a page you can rearrange and printed from the device. Stored on this device, in a folder you pick, or in your own Dropbox.",
    id: base,
    start_url: base,
    scope: base,
    display: "standalone",
...
// GENERATED — do not edit. Emitted by pwa-plugin.ts for the Invoice PWA.
  ...
  return {
    ...
    transformIndexHtml(): HtmlTagDescriptor[] {
      return [
        ...
        {
          tag: "meta",
          attrs: { name: "apple-mobile-web-app-title", content: "Invoice" },
          injectTo: "head",
        },
      ];
    },
